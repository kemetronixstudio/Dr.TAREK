(function(){
  ...
  const I18N = {
    en: {
      ...
      playStudentClassPlaceholder:'Class / Course',
      playOutsideStudent:'I am not in your class / course / school',
      playIdentityRule:'Name is required. Student ID is optional. Class is required unless the outside-student option is checked.',
    ...
    ar: {
      ...
      playStudentClassPlaceholder:'الفصل / الكورس',
      playOutsideStudent:'أنا لست في فصلك / كورسك / مدرستك',
      playIdentityRule:'الاسم مطلوب. رقم الطالب اختياري. الفصل مطلوب إلا إذا تم تحديد خيار الطالب الخارجي.',
  ...
  function buildIdentity(){ const name = String($('playStudentName')?.value || '').trim(); const studentId = String($('playStudentId')?.value || '').trim(); const className = String($('playStudentClass')?.value || '').trim(); const isGuest = !!$('playStudentGuest')?.checked; const grade = String($('playStudentGrade')?.value || 'KG1').trim(); if (!name) throw new Error(tr('playEnterName')); if (!isGuest && !className) throw new Error(tr('playIdentityRule')); return { name, studentId, grade, isGuest, className: isGuest ? 'Guest' : className }; }
  ...
  function init(){ if (document.body.dataset.page !== 'playtest') return; soundEnabled = loadSoundSetting(); autoNextEnabled = loadAutoNextSetting(); applyPlayTranslations(); wireOptionFallbacks(); const guestToggle = $('playStudentGuest'); const classInput = $('playStudentClass'); if (guestToggle && classInput) { const syncGuestState = function(){ const disabled = !!guestToggle.checked; classInput.disabled = disabled; if (disabled) classInput.value = ''; }; guestToggle.addEventListener('change', syncGuestState); syncGuestState(); } loadLeaderboard().catch(error => setStatus(error.message || tr('playNoLeaderboard'))); $('playStartBtn')?.addEventListener('click', startOrResume); $('playNextBtn')?.addEventListener('click', nextQuestion); $('playAgainBtn')?.addEventListener('click', function(){ if ($('playStudentName') && state && state.identity) $('playStudentName').value = state.identity.name || ''; if ($('playStudentId') && state && state.identity) $('playStudentId').value = state.identity.studentId || ''; if ($('playStudentGrade') && state && state.identity) $('playStudentGrade').value = state.identity.grade || 'KG1'; if ($('playStudentClass') && state && state.identity && !state.identity.isGuest) $('playStudentClass').value = state.identity.className || ''; if ($('playStudentGuest') && state && state.identity) $('playStudentGuest').checked = !!state.identity.isGuest; if (guestToggle && classInput) { classInput.disabled = !!guestToggle.checked; if (guestToggle.checked) classInput.value = ''; } clearLocal(); stopTimer(); answerLock = false; state = null; showSection('playStartCard'); setStatus(tr('playReady')); loadLeaderboard().catch(()=>{}); }); $('refreshPlayLeadersBtn')?.addEventListener('click', ()=> loadLeaderboard().catch(()=>{})); $('playSoundToggleBtn')?.addEventListener('click', ()=>{ soundEnabled=!soundEnabled; saveSoundSetting(); updateSoundButton(); }); $('playAutoNextToggle')?.addEventListener('change', function(){ autoNextEnabled=!!this.checked; saveAutoNextSetting(); updateAutoNextToggle(); }); window.kgPlayHandleLangChange = function(){ applyPlayTranslations(); if (state) { renderQuestion(); } loadLeaderboard().catch(()=>{}); } }
