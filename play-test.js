(function(){
  ...
  let state = null, timerId = null, soundEnabled = true, autoNextEnabled = true, answerLock = false, autoNextTimer = null, playDeadlineTs = 0;
  ...
  function stopTimer(){ if (timerId) { clearInterval(timerId); timerId = null; } if (autoNextTimer) { clearTimeout(autoNextTimer); autoNextTimer = null; } playDeadlineTs = 0; }
  function startTimer(){ stopTimer(); if (!state) return; const startLeft = Math.max(0, Number(state.timeLeft || QUESTION_SECONDS) || QUESTION_SECONDS); state.timeLeft = startLeft; playDeadlineTs = Date.now() + (startLeft * 1000); updateBadges(); timerId = setInterval(async function(){ if (!state) return stopTimer(); state.timeLeft = Math.max(0, Math.ceil((playDeadlineTs - Date.now()) / 1000)); updateBadges(); if (state.timeLeft <= 0) { stopTimer(); await finishQuiz(true, false); } }, 250); }
