(function(){
  ...
  let state = null, timerId = null, soundEnabled = true, autoNextEnabled = true, answerLock = false, autoNextTimer = null, playDeadlineTs = 0, timerVersion = 0;
  ...
  function startTimer(){ stopTimer(); if (!state) return; const version = timerVersion; const startLeft = Math.max(0, Number(state.timeLeft || QUESTION_SECONDS) || QUESTION_SECONDS); state.timeLeft = startLeft; playDeadlineTs = Date.now() + (startLeft * 1000); updateBadges(); timerId = setInterval(async function(){ if (!state || version !== timerVersion) return stopTimer(); const remaining = Math.max(0, Math.floor((playDeadlineTs - Date.now() + 999) / 1000)); if (remaining !== state.timeLeft) { state.timeLeft = remaining; updateBadges(); } if (remaining <= 0) { stopTimer(); await finishQuiz(true, false); } }, 200); }
