(function(){
  ...
  const STORAGE_KEY = 'kgPlayTestSessionV12';
  ...
  const STAGE_POOL_SIZE = { starter:320, explorer:520, champion:780 };
  ...
  function renderQuestion(){ if (!state) return; answerLock = false; if (autoNextTimer) { clearTimeout(autoNextTimer); autoNextTimer = null; } if (state.currentIndex >= state.questions.length) { finishQuiz(false, false).catch(()=>{}); return; } showSection('playQuizCard'); const q = state.questions[state.currentIndex]; state.timeLeft = QUESTION_SECONDS; updateBadges(); $('playQuestionText').textContent = q.text; $('playOptions').innerHTML = q.options.map((opt, idx) => `<button type="button" class="play-option-btn" data-option-index="${idx}" onclick="window.__playChooseAnswer(${idx})">${escapeHtml(opt)}</button>`).join(''); $('playNextBtn').disabled = true; startTimer(); }
  function nextQuestion(){ if (!state) return; if (!state.answers.find(a => a.index === state.currentIndex)) return; state.currentIndex += 1; saveProgress().catch(()=>{}); renderQuestion(); }
  async function finishQuiz(timeUp, wrongStop){ if (!state || state.completed) return; state.completed = true; stopTimer(); const score = Number(state.score || 0); const badge = getBadgeMeta(score); const result = await request('?action=submit', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ identity:state.identity, sessionId:state.sessionId, result:{ score, total:state.answers.length, percent: score, answers:state.answers, questionCount:state.answers.length, completedAt:new Date().toISOString(), quizLevel:'Play & Test', stage:state.stage, stageLabel:getStageLabel(state.stage), badgeTitle:badge.title }, progress:{ completed:true, currentIndex:state.currentIndex, questions:state.questions.map(questionPayload), usedQuestionIds:state.usedQuestionIds || state.questions.map(q => q.id).filter(Boolean) } }) }); playFinish(); $('playResultMedal').textContent = badge.medal; $('playResultBadge').textContent = badge.title; $('playResultBadge').className = 'play-result-badge ' + badge.cls; $('playResultScore').textContent = String(score); $('playResultText').textContent = timeUp ? tr('playTimeOverResult',{score}) : wrongStop ? tr('playWrongResult',{score}) : tr('playGreatResult',{score}); saveLocal(); showSection('playResultCard'); if (result && result.leaderboard) renderLeaderboard(result.leaderboard); else loadLeaderboard().catch(()=>{}); }
