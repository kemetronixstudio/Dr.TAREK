...
function verifyToken(token) {
  ...
  let expectedBuffer;
  try {
    sigBuffer = Buffer.from(String(sig || ''), 'base64url');
    expectedBuffer = Buffer.from(String(expected || ''), 'base64url');
    if (!sigBuffer.length || sigBuffer.length !== expectedBuffer.length) return null;
