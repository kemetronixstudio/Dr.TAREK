...
const TOKEN_SECRET = process.env.ACCESS_ACCOUNTS_SESSION_SECRET || process.env.SESSION_SECRET || (globalThis.__KG_ACCESS_FALLBACK_SECRET__ = globalThis.__KG_ACCESS_FALLBACK_SECRET__ || crypto.randomBytes(32).toString('hex'));
...
function sanitizeStoredEditableAccount(raw) {
  ...
  let passwordHash = String(raw.passwordHash || raw.pass || '').trim();
  if (passwordHash && !passwordHash.startsWith('pbkdf2$')) passwordHash = pbkdf2Hash(passwordHash);
...
async function authenticate(user, password) {
  ...
  const ok = verifyPassword(password, match.passwordHash);
  if (!ok) return null;
