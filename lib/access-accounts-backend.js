...
const IS_PRODUCTION_LIKE = process.env.VERCEL === '1' || process.env.NODE_ENV === 'production';
const TOKEN_SECRET = process.env.ACCESS_ACCOUNTS_SESSION_SECRET || process.env.SESSION_SECRET || (IS_PRODUCTION_LIKE ? '' : 'development-only-secret');
...
function writeFileJson(value) {
  if (IS_PRODUCTION_LIKE) return false;
...
function requireTokenSecret() {
  if (!TOKEN_SECRET) {
    const error = new Error('ACCESS_ACCOUNTS_SESSION_SECRET is required in production.');
    error.status = 500;
    throw error;
  }
  return TOKEN_SECRET;
...
async function authenticateFromHeaders(req) {
  if (process.env.ACCESS_ALLOW_HEADER_FALLBACK !== '1') return null;
...
async function saveAccount(payload, actor) {
  ...
  const pass = String(typeof payload.pass !== 'undefined' ? payload.pass : (payload.password || '')).trim();
...
async function changePassword(payload, actor) {
  ...
  const user = String(payload.user || '').trim();
  const nextPass = String(typeof payload.pass !== 'undefined' ? payload.pass : (payload.password || '')).trim();
  if (!user || !nextPass) {
