...
function buildAnalyticsFromStore(store, filters) {
  ...
  resultRows.forEach((row) => {
    ...
    const studentKey = row.identityKey || [row.grade, row.className, row.studentId || row.studentName].join('::');
...
function normalizePlayPlayerKey(row) {
  const studentId = String((row && row.studentId) || (row && row.identity && row.identity.studentId) || '').trim().toLowerCase();
  if (studentId) return `id:${studentId}`;
  const name = String((row && row.studentName) || (row && row.identity && row.identity.name) || '').trim().toLowerCase().replace(/\s+/g, ' ');
  return `name:${name}`;
}
...
async function getPlayLeaderboard() {
  ...
  relevant.forEach((row) => {
    const key = normalizePlayPlayerKey(row);
    ...
    if (rowScore > item.bestScore || (rowScore === item.bestScore && rowPercent > item.bestPercent)) {
      item.bestPercent = rowPercent;
      item.bestScore = rowScore;
      item.questionCount = Number(row.questionCount || 0) || 0;
    }
  });
  const allPlayers = Object.values(playerMap).sort((a, b) => b.bestScore - a.bestScore || b.bestPercent - a.bestPercent || String(b.updatedAt || '').localeCompare(String(a.updatedAt || '')));
...
async function resetPlayLeaderboard() {
  ...
  Object.keys(store.results || {}).forEach((key) => {
    const item = store.results[key];
    if (item && String(item.quizKey || '').startsWith('PLAYTEST|')) {
      delete store.results[key];
      removedResults += 1;
    }
  });
  Object.keys(store.sessions || {}).forEach((key) => {
    const item = store.sessions[key];
    if (item && String(item.quizKey || '').startsWith('PLAYTEST|')) {
  ...
  await writeStore(store);
  return { ok: true, removedResults, removedSessions };
}
