...
async function getPlaySession(identityInput, sessionIdInput) {
  const identity = sanitizeIdentity(Object.assign({}, identityInput || {}, { grade: 'PLAY', isGuest: true, className: (identityInput && identityInput.className) || 'Play & Test' }));
...
async function getPlayLeaderboard() {
  ...
  relevant.forEach((row) => {
    const key = row && row.identity && row.identity.identityKey ? row.identity.identityKey : buildCompositeKey(sanitizeIdentity(row.identity || row), row.quizKey || '');
    ...
    if ((Number(row.percent || 0) || 0) > item.bestPercent) {
      item.bestPercent = Number(row.percent || 0) || 0;
      item.bestScore = Number(row.score || 0) || 0;
      item.questionCount = Number(row.questionCount || 0) || 0;
    }
  });
  const leaderboard = Object.values(playerMap).sort((a, b) => b.bestPercent - a.bestPercent || b.bestScore - a.bestScore || String(b.updatedAt || '').localeCompare(String(a.updatedAt || ''))).slice(0, 10);
  return { ok: true, top3: leaderboard.slice(0, 3), leaderboard, totalPlayers: leaderboard.length };
}
