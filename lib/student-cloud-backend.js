...
async function resetPlayLeaderboard() {
  const store = await readStore();
  const resultKeys = Object.keys(store.results || {});
  resultKeys.forEach((key) => {
    const row = store.results[key];
    if (row && String(row.quizKey || '').startsWith('PLAYTEST|')) delete store.results[key];
  });
  const sessionKeys = Object.keys(store.sessions || {});
  sessionKeys.forEach((key) => {
    const row = store.sessions[key];
    if (row && String(row.quizKey || '').startsWith('PLAYTEST|')) delete store.sessions[key];
  });
  const noteKeys = Object.keys(store.notes || {});
  noteKeys.forEach((key) => {
    if (String(key || '').startsWith('PLAYTEST|')) delete store.notes[key];
  });
  await writeStore(store);
  return { ok: true };
}
