...
function applyTranslations(){
  ...
  if(document.getElementById('teacherQuestionPickerWrap')) renderTeacherQuestionPicker(); wireCollapseButtons(); bindAdminCollapseFallbacks(); wireQuestionFilterButtons(); const cm=document.querySelector('[data-section-key="classManager"]'); if(cm){ cm.classList.remove('hidden'); cm.style.display=''; }
...
function bindAdminCollapseFallbacks(){
  ADMIN_COLLAPSIBLE_CONFIGS.forEach(cfg => {
    const button = document.getElementById(cfg.buttonId);
    if (!button || button.dataset.fallbackWired) return;
    button.dataset.fallbackWired = '1';
...
initLangButtons(); applyTranslations(); renderHomeProgress(); initQuiz(); renderCertificate(); initAdmin(); wireCollapseButtons(); bindAdminCollapseFallbacks(); registerPwa();
