...
(function(){
  ...
  function updateModeHint(){
    ...
    else if(mode === 'endless') hint.textContent = 'Endless: elapsed timer counts up until you finish all questions.';
  ...
  function startEndlessTimer(){
    stopTotalTimer();
    if(APP.playGameMode !== 'endless'){
      writeModeProgress();
      return;
    }
    APP.playTotalTimerStartedAt = Date.now();
    const run = ()=>{
      const elapsedMs = Math.max(0, Date.now() - APP.playTotalTimerStartedAt);
      writeTimerValue(Math.floor(elapsedMs/1000));
      writeModeProgress();
    };
    run();
  ...
  function writeModeProgress(){
    ...
    if(APP.playGameMode === 'total_timer'){
      ...
      const elapsed = Math.max(0, Math.floor((Date.now() - (APP.playTotalTimerStartedAt || Date.now()))/1000));
  ...
  function patchStartFlow(){
    ...
    startBtn.addEventListener('click', ()=>{
      ...
      setTimeout(()=>{
        ...
        if(APP.playGameMode === 'endless') startEndlessTimer();
      }, 0);
    }, true);
  }
