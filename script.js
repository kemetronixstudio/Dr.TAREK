...
async function makeCertificatePdfBlob(){ const area = $('#certificateArea'); if (!area || typeof html2canvas === 'undefined') throw new Error('Certificate tools are unavailable.'); const jspdfLib = window.jspdf; if (!jspdfLib || typeof jspdfLib.jsPDF !== 'function') throw new Error('PDF export is unavailable on this page.'); const canvas = await html2canvas(area, {scale:2, backgroundColor:'#fffdf4', useCORS:true}); const imgData = canvas.toDataURL('image/png'); const { jsPDF } = jspdfLib; const pdf = new jsPDF('p','pt','a4'); const pageWidth = pdf.internal.pageSize.getWidth(); const pageHeight = pdf.internal.pageSize.getHeight(); const ratio = Math.min((pageWidth-40)/canvas.width, (pageHeight-40)/canvas.height); const w = canvas.width*ratio, h = canvas.height*ratio; pdf.addImage(imgData,'PNG',(pageWidth-w)/2,20,w,h); return pdf.output('blob'); }
