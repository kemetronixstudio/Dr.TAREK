...
function applyTranslations(){
  ...
  if(document.getElementById('teacherQuestionPickerWrap')) renderTeacherQuestionPicker();
...
function toggleCollapse(targetId, button){
  ...
  box.classList.toggle('collapsed-body', collapsed);
  box.hidden = collapsed;
  box.style.display = collapsed ? 'none' : '';
  if(button){
    const key = targetId==='questionBankEditorBody' ? (collapsed?'expandEditor':'collapseEditor') : (collapsed?'expandQuestions':'collapseQuestions');
    button.textContent = (translations[getLang()] && translations[getLang()][key]) || button.textContent;
