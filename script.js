...
function applyTranslations(){ const lang = getLang(); document.querySelectorAll('[data-i18n]').forEach(el=>{ const key = el.dataset.i18n; if (translations[lang][key]) el.textContent = translations[lang][key]; }); document.querySelectorAll('[data-placeholder-i18n]').forEach(el=>{ const key = el.dataset.placeholderI18n; if (translations[lang][key]) el.placeholder = translations[lang][key]; }); }
...
function initAdmin(){ if (document.body.dataset.page !== 'admin') return; const loginCard = $('#adminLoginCard'); const panel = $('#adminPanel'); $('#adminLoginBtn')?.addEventListener('click', ()=>{ const user = $('#adminUser').value.trim(); const pass = $('#adminPass').value.trim(); const ok = ADMINS.some(a => a.user.toLowerCase() === user.toLowerCase() && a.pass === pass); if (!ok){ alert('Wrong admin name or password.'); return; } loginCard.classList.add('hidden'); panel.classList.remove('hidden'); populateDashboardDateFilter(); renderAdminDashboard(); renderLevelVisibilityEditor(); renderTimerSettingsEditor(); renderQuizAccessEditor(); renderTeacherTestEditor(); }); $('#addQuestionBtn')?.addEventListener('click', addCustomQuestion); $('#showStoredQuestionsBtn')?.addEventListener('click', renderStoredQuestions); $('#saveLevelsBtn')?.addEventListener('click', saveLevelVisibilityFromAdmin); $('#resetLevelsBtn')?.addEventListener('click', resetLevelVisibilityFromAdmin); $('#saveTimerSettingsBtn')?.addEventListener('click', saveTimerSettingsFromAdmin); $('#resetTimerSettingsBtn')?.addEventListener('click', resetTimerSettingsFromAdmin); $('#dashboardDateFilter')?.addEventListener('change', renderAdminDashboard); $('#exportDataBtn')?.addEventListener('click', ()=>{ const data = {progress:getProgress(), records:getRecords(), attemptsLog:getAttemptsLog(), analytics:getAnalytics(), customQuestions:getCustomQuestions(), questionOverrides:getQuestionOverrides(), levelVisibility:getLevelVisibility()}; const blob = new Blob([JSON.stringify(data,null,2)], {type:'application/json'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='kg-app-data.json'; a.click(); setTimeout(()=>URL.revokeObjectURL(url),1500); }); $('#exportExcelBtn')?.addEventListener('click', exportDashboardExcel); $('#exportJpegBtn')?.addEventListener('click', exportDashboardJpeg); $('#resetDashboardDataBtn')?.addEventListener('click', resetDashboardData); $('#saveQuizPasswordBtn')?.addEventListener('click', saveQuizAccessFromAdmin); $('#clearQuizPasswordBtn')?.addEventListener('click', clearQuizAccessFromAdmin); $('#saveTeacherTestBtn')?.addEventListener('click', saveTeacherTestFromAdmin); $('#clearTeacherTestBtn')?.addEventListener('click', clearTeacherTestFromAdmin); document.getElementById('testMode')?.addEventListener('change', renderTeacherQuestionPicker); document.getElementById('testGrade')?.addEventListener('change', renderTeacherTestEditor); document.getElementById('testQuestionList')?.addEventListener('input', renderTeacherQuestionPicker); document.getElementById('teacherQuestionPickerList')?.addEventListener('change', (e)=>{ if (e.target && e.target.classList.contains('teacher-question-check')) syncTeacherQuestionTextarea(); }); document.getElementById('selectAllTeacherQuestionsBtn')?.addEventListener('click', ()=>{ document.querySelectorAll('.teacher-question-check').forEach(cb => cb.checked = true); syncTeacherQuestionTextarea(); }); document.getElementById('clearTeacherQuestionsBtn')?.addEventListener('click', ()=>{ document.querySelectorAll('.teacher-question-check').forEach(cb => cb.checked = false); syncTeacherQuestionTextarea(); }); }
function normalizeAttemptsLog(){ const current = getAttemptsLog(); if (current.length) return current; const records = getRecords(); const generated = []; Object.values(records).forEach(r => { (r.history || []).forEach(h => { const parts=String(h.date||'').split('/'); const isoDate = parts.length===3 ? `${parts[2]}-${parts[1]}-${parts[0]}` : ''; generated.push({studentName:r.name, grade:h.grade || r.grade || '', percent:h.percent || 0, score:'', quizLevel:'', questionCount:'', date:h.date || '', isoDate, weaknesses:h.weaknesses || r.weakAreas || [], missedQuestions:h.missedQuestions || []}); }); }); if (generated.length) setAttemptsLog(generated); return generated; }
function populateDashboardDateFilter(){ const select = $('#dashboardDateFilter'); if (!select) return; const logs = normalizeAttemptsLog(); const dates = [...new Set(logs.map(x => x.isoDate).filter(Boolean))].sort().reverse(); const current = select.value || 'all'; select.innerHTML = `<option value="all">All Dates</option>` + dates.map(d => `<option value="${d}">${d}</option>`).join(''); select.value = dates.includes(current) || current==='all' ? current : 'all'; }
function getFilteredDashboardData(){ const logs = normalizeAttemptsLog(); const selected = $('#dashboardDateFilter')?.value || 'all'; const filtered = selected === 'all' ? logs : logs.filter(x => x.isoDate === selected); const grouped = {}; filtered.forEach(item => { const key = (item.studentName || '').trim().toLowerCase(); if (!key) return; if (!grouped[key]) grouped[key] = {name:item.studentName, grade:item.grade || '-', attempts:0, best:0, last:0, weakAreas:[]}; grouped[key].attempts += 1; grouped[key].best = Math.max(grouped[key].best, item.percent || 0); grouped[key].last = item.percent || 0; grouped[key].grade = item.grade || grouped[key].grade; grouped[key].weakAreas = Array.from(new Set([...(grouped[key].weakAreas || []), ...((item.weaknesses || []).slice(0,3))])); }); const questionMisses = {}; const skillMisses = {}; filtered.forEach(item => { (item.missedQuestions || []).forEach(q => { questionMisses[q] = (questionMisses[q] || 0) + 1; }); (item.weaknesses || []).forEach(skill => { skillMisses[skill] = (skillMisses[skill] || 0) + 1; }); }); return {selected, filtered, groupedRows:Object.values(grouped), questionMisses, skillMisses}; }

function resetDashboardData(){
  const ok = window.confirm(getLang()==='ar' ? 'هل تريد مسح كل بيانات النتائج ولوحة التحكم؟' : 'Do you want to clear all result and dashboard data?');
  if (!ok) return;
  [storeKeys.progress, storeKeys.records, storeKeys.analytics, storeKeys.attemptsLog, storeKeys.cert].forEach(key => { if (key) localStorage.removeItem(key); });
...
function renderTeacherQuestionPicker(){
  ...
  }).join('') || '<div class="teacher-question-row"><span>No questions found for this grade.</span></div>';
}
...
function saveTeacherTestFromAdmin(){
  ...
  if (mode === 'manual' && !textList.length){
