...
function applyTranslations(){
  ...
  if(document.getElementById('teacherQuestionPickerWrap')) renderTeacherQuestionPicker(); wireCollapseButtons();
...
const PERMISSIONS = ['dashboard','levelVisibility','timerSettings','quizAccess','teacherTest','bulkQuestions','questionBank','accountManager'];
...
function saveAccessAccountFromAdmin(){
  ...
  const role=(document.getElementById('accessAccountRole')?.value||'user').trim();
  if(!user||!pass){ alert(getLang()==='ar'?'اكتب اسم المستخدم وكلمة المرور.':'Enter username and password.'); return; }
  ...
  const accounts=getAccessAccounts().filter(a=>a.user.toLowerCase()!==user.toLowerCase());
  accounts.push({user,pass,role,permissions}); setAccessAccounts(accounts);
  document.getElementById('accessAccountUser').value=''; document.getElementById('accessAccountPass').value=''; document.getElementById('accessAccountRole').value='user';
  renderAccessPermissions([]); renderAccessAccountsList(); alert(getLang()==='ar'?'تم حفظ الحساب.':'Account saved.');
}

function wireCollapseButtons(){
  const editorBtn = document.getElementById('toggleQuestionBankEditorBtn');
  const existingBtn = document.getElementById('toggleExistingQuestionsBtn');
  ...
  if (existingBtn && !existingBtn.dataset.wired){
    existingBtn.dataset.wired = '1';
    existingBtn.addEventListener('click', ()=>toggleCollapse('questionEditorListWrap', existingBtn));
  }
}
...
function toggleCollapse(targetId, button){
  ...
  if (!box) return;
  const shouldCollapse = !box.classList.contains('collapsed-body');
  if (shouldCollapse){
  ...
  if (button){
    const key = targetId === 'questionBankEditorBody'
      ? (shouldCollapse ? 'expandEditor' : 'collapseEditor')
      : (shouldCollapse ? 'expandQuestions' : 'collapseQuestions');
    if (translations[getLang()] && translations[getLang()][key] !== undefined){
      button.textContent = translations[getLang()][key];
    }
